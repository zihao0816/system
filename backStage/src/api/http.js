...
export function fetch(url, params = {}) {
    return new Promise((resolve, reject) => {
        axios.get(url, { params: params, headers: { "sessionId": sessionStorage.sessionId } })
            .then(response => {
...
export function doRequest(url, data, success, error, thenfunction, ContentType, getpost,catchs, config) {
    ...
    return axios(config)
        .then(res => {
            if (res.status != 200) {
                console.log('请求失败，请稍后再试','HTTP.JS---121')
            } else {
...
var password = md5('haozhongyi201861')
var _this = this;
    doRequest('/api/sysuserinfo/login',{usercode:'haozhongyi',password:password},function(res){
